...
$runnerPath = Join-Path $repositoryRoot 'scripts\gateway237-personal-wechat-adapter-deploy.cjs'
...
if (-not (Test-Path -LiteralPath $runnerPath -PathType Leaf)) {
  throw 'The controlled adapter deploy runner is unavailable.'
}
...
try {
  ...
  $start.EnvironmentVariables['GAIOP_WEIXIN_DEPLOY_SSH_HOST'] = [string]$stored.Host
  $start.EnvironmentVariables['GAIOP_WEIXIN_DEPLOY_SSH_USERNAME'] = [string]$stored.Username
  $start.EnvironmentVariables['GAIOP_WEIXIN_DEPLOY_SSH_PASSWORD'] = [Runtime.InteropServices.Marshal]::PtrToStringBSTR($bstr)
  $start.EnvironmentVariables['GAIOP_WEIXIN_DEPLOY_ROOT'] = $repositoryRoot
  ...
  if ($process.ExitCode -ne 0 -or -not $result.completed) {
    throw "The controlled adapter deploy failed: $($result.status); remoteExit=$($result.remoteExitCode)"
  }
