...
export function toAlertNotificationPreferences(row) {
  ...
  for (const [field, column] of Object.entries(FIELD_TO_COLUMN)) value[field] = Boolean(row[column])
  return value
}
...
export function validateAlertNotificationPreferences(input) {
  ...
  for (const field of FIELDS) {
    if (typeof input[field] !== 'boolean') return { ok: false, error: '告警通知设置必须使用布尔值' }
  }
...
export function saveAlertNotificationPreferences(db, userId, settings, now = Date.now()) {
  const values = FIELDS.map((field) => settings[field] ? 1 : 0)
  db.prepare(`
    INSERT INTO alert_notification_preferences (
      user_id, realtime_enabled, sound_enabled, minor_popup_enabled, minor_notification_enabled,
