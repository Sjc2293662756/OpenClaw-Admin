...
### 6.1 遗留代码依赖评估与分阶段整理（非当前安全待办）

当前正式状态是模块、菜单和前端路由隐藏，服务端九组遗留前缀最前置统一返回 410，并由自动化不可达测试防止后方旧 handler 被误开放。这已经满足当前私有化和公网部署的安全边界，遗留源码物理存在不再列为安全缺口或近期交付项。

考虑到旧 handler、前端组件、PTY/桌面依赖、旧备份结构、数据库表及共享工具可能与正式能力存在间接依赖，当前不直接删除。后期仅在 ISO 冻结、依赖漏洞治理或维护需要时，先完成引用和启动副作用评估，再逐组、小范围整理；不一次性批量删除，不随源码整理自动删除数据库结构或生产数据，并继续保留 410 回归测试。
...
后续任务不是为了补上本轮尚未生效的安全措施。已不可达源码仅保留为后期依赖评估和分阶段整理事项；当前需要继续建设的是本轮明确不在范围内的“权限拒绝审计”能力。
