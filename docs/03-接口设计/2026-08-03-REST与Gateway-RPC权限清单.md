...
> **2026-08-31 补充：** 本清单的方法/动作/数据边界仍是最终边界；其上新增服务端单用户模块门禁。deny 先拒绝相关页面、REST 和 RPC，allow 只打开固定目录中登记的安全能力，不改变本文中管理员写操作、报告/会话归属、敏感投影和退役 410。固定 key、导航/路由/REST/SSE/RPC 映射及风险范围见[2026-08-31-单用户模块权限覆盖设计](../04-公共设计/2026-08-31-单用户模块权限覆盖设计.md)。
...
| 对象 | 数量 | 说明 |
|---|---:|---|
| 正式 REST 方法+路径 | 69 | 第 2 节逐项列出，不把同一路径的不同 HTTP 方法合并；新增 4 个会话留存接口和 1 个水位只读接口 |
...
| GET `/api/events` | 实时事件 | 是 | — | 本人 Gateway 事件；告警只读 | 本人 Gateway 事件；告警只读 | 全量只读事件和告警 | 全量事件和告警 | 会话事件流、告警页面模型和非敏感告警流状态 | 保留；Bearer fetch SSE、心跳、会话隔离和告警角色过滤 |
...
| GET `/api/alerts` | 告警 | 是 | — | 只读 | 只读 | 全量只读 | 全量 | 告警业务数据 | 系统级列表，不按用户归属隔离 |
...
| GET `/api/system/storage-watermarks` | 系统监控 | 是 | — | 403 | 只读 | 全量只读 | 全量 | 受管文件系统脱敏标识、当前水位和受限告警；不返回路径或文件信息 | 与系统监视器角色边界一致；无清理能力 |
...
| 告警事件 | 阶段四消费 `alert` / `alertStreamState`，当前前端可忽略未知类型 | 四类登录角色允许；告警投影与 `GET /api/alerts` 一致 | 基础用户仍不得获得其它无归属敏感 SSE 数据 |
