...
| 创建日期 | 2026-07-16 |
| 最后更新 | 2026-07-30 |
| 状态 | v0.1历史契约；已上线部分以本补充、模块文档和最新发布记录为准 |
...
> **v0.1后续实现补充：** 报告正式目录与Web/外部频道来源已在237上线；Web来源从 `chat.send.metadata` 改为服务器端签名快照，管理字段不进入模型数据面。告警查询已改为先过滤保留历史再分页。会话标题、排序、按 `sessionKey` 的实时隔离及默认运行会话隔离均已发布。下文对应“待完成”或 metadata 方案保留为早期契约轨迹，不得覆盖2026-07-24至30的发布记录。后续若继续扩展跨仓字段，应新建正式契约版本，不再扩大 v0.1。
